...
def get_head_loss(head_type):
    head_type2loss_fn = {
        'classification': F.cross_entropy,
        'regression': mae_loss,
        'mean_variance': mean_variance_loss,
        'unimodal_concentrated': unimodal_concentrated_loss_not_official,
        'dldl': dldl_loss,
        'dldl_v2': dldl_v2_loss,
        'soft_labels': soft_labels_loss,
        'megaage': megaage_loss,
        'orcnn': orcnn_loss,
        'extended_binary_classification': coral_loss,
        'coral': coral_loss,
    }

    if head_type not in head_type2loss_fn.keys():
        raise NotImplementedError()
    
    return head_type2loss_fn[head_type]
...
def mean_variance_loss(logits: torch.Tensor, labels: torch.Tensor, lambda_1=0.2, lambda_2=0.05):
    ...
    Args:
        logits (torch.tensor): Size(batch_size, num_classes])
    ...
    class_labels = torch.arange(0, num_classes)
...
def unimodal_concentrated_loss_not_official(logits: torch.Tensor, labels: torch.Tensor, lambda_=1000):
    """
    Computes the Unimodal-Concentrated Loss as defined in Unimodal-Concentrated Loss: Fully Adaptive Label Distribution Learning for Ordinal Regression. Unofficial implementation.
    ...
    for label in labels:
        select.append(torch.tensor(
            [i for i in range(label)] + [i for i in range(label+1, num_classes)]))
...
def orcnn_loss(logits: torch.Tensor, labels: torch.Tensor, lambdas=None):
    ...
    Args:
        logits (torch.tensor): Size(batch_size, num_classes - 1])
        labels (torch.tensor): Size([batch_size])
    ...
    if lambdas is None:
        lambdas = torch.ones([num_classes - 1]).to(labels.device)
    ...
    for label in labels:
        ...
        extended_label = [1]*int_label + [0] * (num_classes - 1 - int_label)
...
def megaage_loss(logits: torch.Tensor, labels: torch.Tensor, truncation_distance: int = 3, sigma: float = 2.):
    ...
    Args:
        logits (torch.tensor): Size(batch_size, num_classes + num_classes - 1]) Contains [num_classes] logits of output distribution and [num_classes-1] binary probabilities for an extended binary classification task.
        labels (torch.tensor): Size([batch_size]) 
    ...
    """
    batch_size = logits.shape[0]
    num_classes = int((logits.shape[1] + 1) / 2)
    logits, binary_probas = logits[:, :num_classes], logits[:, num_classes:]
    probas = F.softmax(logits, dim=1)
    ...
    for label in labels:
        ...
        truncation[max(int_label-truncation_distance, 0)
                       :min(int_label+truncation_distance, num_classes - 1)] = 0
